...
- **Продвинутый буфер обмена в стиле Windows 11**:
  ...
  - Поиск в реальном времени с четкой векторной лупой Fluent Search.
  - Свободное изменение размера окна мышью за края и правый нижний угол с сохранением размеров в реестр.
  - Кастомный темный скроллбар Fluent Scrollbar без белых системных полос.
  - Надежная передача фокуса и вставка **Ctrl+V** в активное поле ввода (при закрепе окно не сворачивается, а поле ввода сохраняет активность).
  - Два режима по клику: **Вставлять сразу (Ctrl+V)** в активное окно или **Только копировать в буфер**.
  - Кнопка быстрого предпросмотра **[👁]** на каждой карточке.
...
- **Встроенное окно детального просмотра (ItemViewer)**:
  - Зум изображений колесиком мыши от 10% до 500% с центровкой на курсор и панорамированием зажатой мышью.
  - Кнопка булавки **[📌]** для фиксации окна поверх всех окон.
  - Встроенный просмотр текстовых файлов (.txt, .log, .cs, .json и др.) с возможностью выделения и частичного копирования.
  - Извлечение чистого текста из документов .docx и .doc без внешних библиотек.
- **Плавающая полка файлов SuperHub**:
  - Компактный ярлычок с акцентным индикатором темы, без серой дымки и без ложных перехватов кликов по рабочему столу.
  - Автоматически выдвигается при зажатии левой кнопки мыши у края экрана во время перетаскивания файлов или при наведении на ярлычок.
  ...
  - Гибкое размещение: по бокам экрана (справа/слева, сверху/по центру/снизу), а также сверху по центру и снизу по центру в горизонтальном виде с прокруткой карточек.
